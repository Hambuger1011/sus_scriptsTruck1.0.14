...
public static class NativeGallery
{
	...
	public enum MediaType { Image = 0, Video = 1, Audio = 2 };
	...
	private static Permission GetMediaFromGallery( MediaPickCallback callback, MediaType mediaType, string mime, string title )
	{
		...
		if( result == Permission.Granted && !IsMediaPickerBusy() )
		{
			...
			if( mediaType == MediaType.Image )
				...
				callback( null );
#elif UNITY_EDITOR
            WindowDll.PickImage(callback, mediaType, mime, title);
#else
            if( callback != null )
				callback( null );
#endif
...
public static class WindowDll
{
    ...
    public static void PickImage(NativeGallery.MediaPickCallback callback, NativeGallery.MediaType mediaType, string mime, string title)
    {
        switch (mediaType)
        {
            case NativeGallery.MediaType.Image:
                {
                    var path = UnityEditor.EditorUtility.OpenFilePanelWithFilters(title, "C:", new string[] { "Image files", "png,jpg,jpeg" });
                    callback(path);
                }
                break;
            default:
                {
                    ...
                    var path = UnityEditor.EditorUtility.OpenFilePanelWithFilters(title, "C:", new string[] { "All files", "*" });//"Image files", "png,jpg,jpeg", 
                    callback(path);
                }
                break;
        }
