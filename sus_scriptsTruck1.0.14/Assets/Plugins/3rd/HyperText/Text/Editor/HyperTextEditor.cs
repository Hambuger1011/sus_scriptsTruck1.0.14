...
namespace Candlelight.UI
{
	...
	public class HyperTextEditor : BaseEditor<HyperText>
	{
		...
		public static void CreateNew(MenuCommand menuCommand)
		{
            ...
            hyperText.Styles = Resources.Load<HyperTextStyles>("Styles/Default");
            hyperText.color = color;
			hyperText.text = "New <a href=\"link\">HyperText</a>";
		...
		)
		{
			...
			else if (inheritingFrom != null)
			{
				//s_ReusableLabel.tooltip = string.Format(
				//	"Overriding Font {0} inherited from {1}.", "xxxx", inheritingFrom.name
				//);
				//status = ValidationStatus.Warning;
			}
			if (
        ...
        [SerializedProperty]
        private SerializedProperty m_TextGeneratorInput;
			...
			if (s_HitboxTogglePreference.CurrentValue)
			{
				...
				foreach (KeyValuePair<HyperText.LinkInfo, List<Rect>> linkHitboxes in m_LinkHitboxes)
				{
					...
					foreach (Rect hitbox in linkHitboxes.Value)
					{
						...
						{
							Handles.DrawLine(vertices[i], vertices[s_QuadIndexWrap[i]]);
						}
		...
		public override void OnInspectorGUI()
		{
            ...
            {
                EditorGUILayout.TextArea(m_TextGeneratorInput.stringValue,GUILayout.ExpandHeight(true), GUILayout.MinHeight(50));
            }
            EditorGUI.EndDisabledGroup();
			...
			position = EditorGUILayout.GetControlRect();
			EditorGUI.PropertyField(position, m_FontStyle);

			position = EditorGUILayout.GetControlRect();
            EditorGUI.PropertyField(position, m_FontSize);

			position = EditorGUILayout.GetControlRect();
            EditorGUI.PropertyField(position, m_LineSpacing);
