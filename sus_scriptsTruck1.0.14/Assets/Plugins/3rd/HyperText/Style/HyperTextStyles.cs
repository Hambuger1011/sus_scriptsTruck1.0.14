...
namespace Candlelight.UI
{
	...
	public class HyperTextStyles : ScriptableObject
	{
		...
		public struct Quad : IIdentifiable<string>, IPropertyBackingFieldCompatible<Quad>
		{
			...
			public float SizeScalar { get { return m_SizeScalar; } set { m_SizeScalar = value; } }
            /// <summary>
            /// Gets the sprite.
            /// </summary>
			...
			public Quad(
				...
				bool shouldRespectColorization = false,
				string linkId = "",
...
#pragma warning restore 219
		...
		private readonly Dictionary<LinkSubclass, HyperTextStyles> m_InheritedLinkStyles = new Dictionary<LinkSubclass, HyperTextStyles>();
		//private readonly Dictionary<Quad, HyperTextStyles> m_InheritedQuadStyles = new Dictionary<Quad, HyperTextStyles>();
		private readonly Dictionary<Text, HyperTextStyles> m_InheritedTextStyles = new Dictionary<Text, HyperTextStyles>();
        ...
        private void AddToCascade<T>(List<T> styles, HyperTextStyles style, Dictionary<T, HyperTextStyles> cascade) where T : IIdentifiable<string>, new()
        {
            List<T> cascadeKeys = new List<T>(cascade.Keys.Count * 2);
            foreach (var k in styles)
            {
                T existingKey = new T();
                cascadeKeys.Clear();
                cascadeKeys.AddRange(cascade.Keys);
                for (int i = 0; i < cascadeKeys.Count; ++i)
                {
                    if (cascadeKeys[i].Identifier.ToLower() == k.Identifier.ToLower())
                    {
                        existingKey = cascadeKeys[i];
                        break;
                    }
                }
                if (existingKey.Identifier.ToLower() == k.Identifier.ToLower())
                {
                    if (
                        existingKey is IPropertyBackingFieldCompatible &&
                        k is IPropertyBackingFieldCompatible &&
                        (existingKey as IPropertyBackingFieldCompatible).GetSerializedPropertiesHash() !=
                        (k as IPropertyBackingFieldCompatible).GetSerializedPropertiesHash()
                    )
                    {
                        cascade.Remove(existingKey);
                        cascade.Add(k, style);
		...
		private void RebuildCascadingAndInheritedStyles()
		{
			...
			foreach (HyperTextStyles styles in m_InheritedStyles)
			{
				...
				AddToCascade(styles.m_LinkStyles, styles, m_InheritedLinkStyles);

				//AddToCascade(styles.m_InheritedQuadStyles, m_InheritedQuadStyles);
				//AddToCascade(styles.m_QuadStyles, styles, m_InheritedQuadStyles);

				AddToCascade(styles.m_InheritedTextStyles, m_InheritedTextStyles);
				AddToCascade(styles.m_CustomTextStyles, styles, m_InheritedTextStyles);
			}
			// rebuild cascades
