...
function string.split(s, sep)
	if not s then
		error(string.format("string.split error %s", debug.traceback()))
	end
...
function string.escapeCharacter(s)
...
local byteCounts = {}
function string.parseStrToList(str)
	local len  = #str
	local left = 0
	...
	while len ~= left do
		...
		left = left + i
		t[#t + 1] = tmpString
		byteCounts[tmpString] = byteCount
	end
	return t
end

function string.isAllSpace(str)
	local arr = string.parseStrToList(str)
	local result = true
	for i=1, #arr do
...
function string.getMd5(value)
	--local buffer = CS.Utility.BytesConvert(value)
	return logic.cs.CFileManager.GetStringMd5(value)
end