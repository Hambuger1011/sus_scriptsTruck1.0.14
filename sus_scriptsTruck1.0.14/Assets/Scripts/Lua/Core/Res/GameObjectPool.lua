...
-- local GameObjectPool = BaseClass("GameObjectPool", core.Singleton)
...
-- 	local args = SafePack(...)
-- 	ResMgr.LoadAsync(ResMgr.tag.Global, ResMgr.type.Prefab, path, function(task)
-- 		local go = task.resPrefab
...
-- 	local task = ResMgr.LoadAsync(ResMgr.tag.Global,ResMgr.type.Prefab, path, progress_callback)
-- 	local go = task.resPrefab
-- 	if not IsNull(go) then
-- 		CacheAndInstGameObject(self, path, go, inst_count)
-- 	end
