...
local function InitEffect(self, go)
	...
	if not self.config.IsLoop and self.config.LiveTime > 0 then
		self.timer = TimerManager:GetTimer(self.config.LiveTime, self.timer_action , self)
		self.timer:Start()
	end
end
...
local function __init(self, parent_trans, effect_config, create_callback)
	...
	if effect_config == nil then
		...
		-- 资源加载
		GameObjectPool:GetGameObjectAsync(effect_config.EffectPath, function(go, self)
			if self ~= nil then
				InitEffect(self, go)
				self:OnCreate()
...
local function __delete(self)
	...
	if effect_config and not IsNull(self.gameObject) then
		GameObjectPool:RecycleGameObject(self.config.EffectPath, self.gameObject)
	end
	-- 释放引用
	self.config = nil
	self.gameObject = nil
