...
function M.RegisterUIPress(target, callBack, data, delay, loop, useFrame)
	...
	if not delay and not loop and not useFrame then
		...
		uiPressMaps[key].timer = core.Timer.New(delay, callBack, data, loop, useFrame);
	end
end
...
function M.OnTouchClick(eventData)
	...
	UIManager:CheckOutClick(eventData.Position);
	UIManager:CheckComponentClick(eventData.Position);
end
