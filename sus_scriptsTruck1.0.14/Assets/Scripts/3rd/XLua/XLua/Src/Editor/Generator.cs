...
namespace CSObjectWrapEditor
{
    public static class GeneratorConfig
    {
        ...
        public static string tools_path = System.Environment.CurrentDirectory + "/Z_Work/Tools/";
    ...
    {
        static HashSet<string> s_genClass = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        static void Clear()
        {
            s_genClass.Clear();
        }
        ...
        static void GenWrap(IEnumerable<Type> types, string save_path)
        {
            ...
            foreach (var wrap_type in types)
            {
                ...
                for (int i = 1; s_genClass.Contains(filePath); ++i)
                {
                    filePath = save_path + wrap_type.ToString().Replace("+", "").Replace(".", "")
                    .Replace("`", "").Replace("&", "").Replace("[", "").Replace("]", "").Replace(",", "") + "Wrap_" + i + ".cs";
                }
                s_genClass.Add(filePath);
...
#pragma warning restore 618
        ...
        public static void GenAll()
        {
            ...
            if (File.Exists(GeneratorConfig.tools_path + "MonoBleedingEdge/bin/mono.exe"))
            {
                GenUsingCLI();
                return;
            }
            ...
            try
            {
                EditorUtility.DisplayProgressBar("XLua", "生成LuaWrap中...", 0);
                Clear();
                var start = DateTime.Now;
                Directory.CreateDirectory(GeneratorConfig.common_path);
        ...
        {
#if UNITY_EDITOR_OSX
            var monoPath = GeneratorConfig.tools_path + "MonoBleedingEdge/bin/mono";
#else
            var monoPath = GeneratorConfig.tools_path + "MonoBleedingEdge/bin/mono.exe";
#endif

            var args = new List<string>()
            {
                GeneratorConfig.tools_path + "XLuaGenerate.exe",
        ...
        public static void CheckGenrate(BuildTarget target, string pathToBuiltProject)
        {
            ...
            {
                throw new InvalidOperationException("点击菜单:MenuItem->XLua->Gen Code");
            }
        }
#endif
    }
