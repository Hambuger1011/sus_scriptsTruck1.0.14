...
public class InfinityGridLayoutGroup :MonoBehaviour
{
    ...
    [SerializeField]
    int minAmount = 0;//子物体的数量   实现无限滚动，需要的最少的child数量。屏幕上能看到的+一行看不到的，比如我在屏幕上能看到 2 行，每一行 2 个。则这个值为 2行*2个 + 1 行* 2个 = 6个。
    ...
    {
        //Debug.Log("dafad7777");
        scrollRect.onValueChanged.RemoveListener((data) => { ScrollCallback(data); });
    }
    ...
    void UpdateChildren()
    {
        ...
        if (gridLayoutGroup.constraint == GridLayoutGroup.Constraint.FixedColumnCount)
        {
            ...
            else
            {
                ...
                if (children[children.Count - 1] == null) return;
    ...
    public void SetAmount(int count)
    {
        ...
        if (scrollRect.gameObject == null)return;
