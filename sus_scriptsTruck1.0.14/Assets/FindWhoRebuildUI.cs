    ...
    private void Update()
    {
        for (int j = 0; j < m_LayoutRebuildQueue.Count; j++)
        {
            ...
            if (ObjectValidForUpdate(rebuild))
            {
                ...
                if (g == null)
                {
                    Debug.LogFormat("{0}引起网格重建", getHierarchy(rebuild.transform));
                }
                else
                {
                    var can = g.canvas;
                    ...
                    {
                        Debug.LogFormat("{0}引起{1}网格重建", getHierarchy(rebuild.transform), can.name);
                    }
                }
        ...
        for (int j = 0; j < m_GraphicRebuildQueue.Count; j++)
        {
            ...
            if (ObjectValidForUpdate(element))
            {
                ...
                if (g == null)
                {
                    Debug.LogFormat("{0}引起网格重建" , getHierarchy(element.transform));
                }
                else
                {
                    var can = g.canvas;
                    ...
                    {
                        Debug.LogFormat("{0}引起{1}网格重建", getHierarchy(element.transform), can.name);
                    }
                }
            }
