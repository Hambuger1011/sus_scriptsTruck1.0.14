...
namespace TPImporter
{
	...
	public class SpriteGeometry
    {
		...
		public void ensureVerticesFitInFrame(Vector2[] vertices, float frameWidth, float frameHeight)
		{
			...
			{
				if (vertices[i].x > frameWidth || vertices[i].y > frameHeight)
				{
		...
		{
			Vector2[] array = this.vertices;
			if ((double)scaleFactor != 1.0)
			{
				array = new Vector2[this.vertices.Length];
				for (int i = 0; i < this.vertices.Length; i++)
				{
					array[i].Set(scaleFactor * this.vertices[i].x, scaleFactor * this.vertices[i].y);
				}
			}
			this.ensureVerticesFitInFrame(array, sprite.rect.width, sprite.rect.height);
