...
public static class MathHelper
{
...
#else
        //var p = camTransform.InverseTransformPoint(worldPos);
        var p = worldPos - camTransform.position;
        if (p.z <= 0)
        {
            return false;
        }
    ...
    {
        Transform subNode = null;
        if (_subObjectNamePath.IndexOf('/') >= 0)
        {
            subNode = _targetObject.transform.Find(_subObjectNamePath);
            if (subNode)
            {
        ...
        if (subNode == null)
        {
            ...
            {
                GameObject subObj = FindSubObject(_targetObject.transform.GetChild(i).gameObject, _subObjectNamePath);
                if (subObj != null)
                {
                    return subObj;
                }
