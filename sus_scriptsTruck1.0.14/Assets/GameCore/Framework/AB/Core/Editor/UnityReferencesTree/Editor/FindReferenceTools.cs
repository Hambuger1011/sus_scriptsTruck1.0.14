...
public class FindReferenceTools : EditorWindow
{
    ...
    [MenuItem("Assets/GameTools/查找依赖", false, MenuPriority.AB + 2000)]
    ...
    public class RefItem
    {
        ...
        public void InitRef(Object o, bool bRecursive = true)
        {
            _object = o;
            ...
            if(bRecursive)
            {
                string[] paths = AssetDatabase.GetDependencies(_path, true);
                for (int i = 0; i < paths.Length; i++)
                {
                    var obj = AssetDatabase.LoadAssetAtPath<Object>(paths[i]);
                    if (obj == o)
