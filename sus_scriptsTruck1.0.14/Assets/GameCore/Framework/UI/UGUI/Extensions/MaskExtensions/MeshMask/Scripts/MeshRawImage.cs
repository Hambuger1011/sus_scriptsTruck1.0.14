...
[AddComponentMenu("UI/Custom/Mesh RawImage")]
[RequireComponent(typeof(RawImage))]
public class MeshRawImage : BaseMeshEffect
{
        ...
        if (this.enabled)
        {
            ...
            if (mask)
            {
...
#if UNITY_EDITOR
                        sw.Stop();
                        LOG.Info("重建Mask网格耗时:" + sw.ElapsedMilliseconds + "ms," + sw.ElapsedTicks + "ticks");
#endif
                    }
                ...
                if (vertices != null && triangles != null)
                {
                    ...
                    Vector4 uv = image.texture != null ? new Vector4(_image.uvRect.yMin, _image.uvRect.xMin, _image.uvRect.yMax, _image.uvRect.xMax) : Vector4.zero;
                    float w = uv.z - uv.x;
                    float h = uv.w - uv.y;
                    float uvScaleX = w / tw;
                    float uvScaleY = h / th;
