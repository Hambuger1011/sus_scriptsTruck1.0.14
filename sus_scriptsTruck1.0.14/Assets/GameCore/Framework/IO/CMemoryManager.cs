...
public class CMemoryManager
{
	...
	public static void WriteLong(long value, byte[] data, ref int offset)
	{
		...
		WriteInt(value3, data, ref offset);
	}
	...
	{
		int result = data[offset + 1] << 8 | data[offset];
		offset += 2;
		return result;
	}
	...
	{
		int result = data[offset + 3] << 24 | data[offset + 2] << 16 | data[offset + 1] << 8 | data[offset];
		offset += 4;
		return result;
	}
	...
	public static long ReadLong(byte[] data, ref int offset)
	{
		...
		int num2 = ReadInt(data, ref offset);
	...
	public static void WriteString(string str, byte[] data, ref int offset)
	{
		...
		WriteShort((short)bytes, data, ref offset);
		offset += bytes;
	}

	public static string ReadString(byte[] data, ref int offset)
	{
		int num = ReadShort(data, ref offset);
	...
	public unsafe static bool Copy(byte[] dst, byte[] src, uint length)
	{
		if (dst.Length >= length && src.Length >= length)
		{
			fixed (byte* dstPtr = dst)
			{
				...
				{
                    //s_copyMemory(new IntPtr(dstPtr), new IntPtr(srcPtr), (int)length);
                    Marshal.Copy(src, 0, new IntPtr(dstPtr), (int)length);
                }
			}
			return true;
		}
