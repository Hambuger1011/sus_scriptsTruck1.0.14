...
public class DrawCurve : MonoBehaviour {
	...
	void Start () {
		...
		controlLine = new VectorLine("Control Line",null, curvePoints, 2.0f);
		controlLine.color = new Color(0.0f, .75f, .1f, .6f);
		controlLine.Draw();
		
		// Make the line object for the curve
		line = new VectorLine("Curve", null, new List<Vector2>(segments+1), lineTexture, 5.0f, LineType.Continuous, Joins.Weld);
