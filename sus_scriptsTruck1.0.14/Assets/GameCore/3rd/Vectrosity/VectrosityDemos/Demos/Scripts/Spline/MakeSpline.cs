...
public class MakeSpline : MonoBehaviour {
	...
	void Start () {
		...
		if (usePoints) {
			var dotLine = new VectorLine("Spline", null, new List<Vector3>(segments+1), 2.0f, LineType.Points);
			dotLine.MakeSpline (splinePoints.ToArray(), segments, loop);
			dotLine.Draw();
		}
		else {
			var spline = new VectorLine("Spline", null, new List<Vector3>(segments+1), 2.0f, LineType.Continuous);
