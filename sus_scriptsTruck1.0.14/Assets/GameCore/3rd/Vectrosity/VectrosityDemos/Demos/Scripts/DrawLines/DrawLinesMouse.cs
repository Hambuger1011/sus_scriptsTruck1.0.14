...
public class DrawLinesMouse : MonoBehaviour {
	...
	void Start () {
		...
		if (line3D) {
			line = new VectorLine("DrawnLine3D", null, new List<Vector3>(), tex, useLineWidth, LineType.Continuous, Joins.Weld);
		}
		else {
			line = new VectorLine("DrawnLine", null, new List<Vector2>(), tex, useLineWidth, LineType.Continuous, Joins.Weld);
		}
