...
using JsonObject = NiceJson.JsonObject;
    /// <summary>
    ...
    public class MobileInputField : MobileInputReceiver {
		...
		public static Rect GetScreenRectFromRectTransform (Canvas canvas, RectTransform rect) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (canvas.renderMode == RenderMode.ScreenSpaceOverlay) {
					...
					screenCoord = RectTransformUtility.WorldToScreenPoint (canvas.worldCamera, corners[i]);
				}
        ...
        private void PrepareNativeEdit () {
            ...
            Rect rect = GetScreenRectFromRectTransform (this.canvas, this._inputObjectText.rectTransform);
	...
	/// <summary>
        ...
        public void SetRectNative (RectTransform inputRect) {
            Rect rect = GetScreenRectFromRectTransform (this.canvas, inputRect);
            if (_lastRect == rect) {
                return;
            }
            _lastRect = rect;
